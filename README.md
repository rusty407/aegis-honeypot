...
- 🔒 **Zero-Execution Sandbox:** Attacker commands are deterministically handled by an async dispatcher. No malicious code is ever executed on the host system. **Note the one deliberate exception:** with `forensics.fetch_payloads = true` (the default) the gateway really does fetch URLs an attacker types into `wget`/`curl`, so that payloads can be captured. Those requests are vetted (no loopback/private/link-local targets, no redirects followed), size-capped and budgeted per session — but they originate from this host's address. Set it to `false` if outbound traffic attributed to you is a problem; the attacker still sees a convincing download either way.
- 📂 **Ephemeral OverlayFS Mount Isolation:** Each connection receives an isolated file system layer backed by a Golden Rootfs, mounted `nodev,nosuid,noexec`. Attacker file modifications (`touch`, `mkdir`, `echo > file`, `wget`) are trapped in private `upperdir` instances and never leak between sessions. Mounts are released on `Drop`, so an abrupt disconnect cannot strand them, and any session directory left behind by a crash or reboot is analysed and reclaimed at the next startup. Containment here comes from the zero-execution dispatcher, not from namespaces — no PTY is allocated and no process is forked, so there is nothing to confine.
- 🕵️ **Anti-Fingerprinting:** Presents an OpenSSH 9.6p1 / Ubuntu 22.04 identification banner, a persistent host key, and authentic Ubuntu 22.04 LTS directory structures (`/proc`, `/sys`, `/etc`, `/var/log`, `/dev`, `/boot`). **This is not yet complete:** the `hasshServer` fingerprint — an MD5 over the algorithm lists in `SSH_MSG_KEXINIT` — still reflects the underlying Rust SSH library's defaults rather than OpenSSH's, and only an Ed25519 host key is offered where real OpenSSH also offers RSA. A determined scanner comparing KEXINIT against a real Ubuntu host will still tell the difference; a banner-grabbing one will not. The emulated shell also has real gaps (no pipes, no `exec` channel, no job control). Do not treat this as proof against a motivated analyst.
...
- ⚡ **Kernel eBPF Telemetry Probes (not currently built):** Aya-based tracepoints for `sys_enter_execve`, `sys_enter_connect` and `sys_enter_memfd_create` exist in `crates/aegis-ebpf/ebpf`, but no build step compiles them and the loader ships an empty bytecode blob — so `SYSCALL_EXECVE`, `SYSCALL_CONNECT` and `SYSCALL_MEMFD_CREATE` events are **never emitted** today. The zero-execution design means there are no sandbox processes to trace in any case. Treat this as roadmap, not a shipped capability, and note that the deployment therefore does not need `CAP_BPF` or `CAP_NET_ADMIN`.
...
Beyond the live feed, the dashboard also gives you:
- **Attack Origins map** — every geo-tagged session plotted by lat/lon on a lightweight grid projection (no map tiles/CDN dependency), bucketed so a busy honeypot shows clusters rather than overlapping pins.
- **Session Archive** — a searchable, paginated history of every session ever seen (independent of the live feed's in-memory window), with duration, credentials, command/payload counts, and a threat score per session.
- **Threat-score leaderboard** — source IPs ranked by cumulative threat score (commands run, credentials harvested, payloads dropped), not just raw event volume, to surface attackers who actually did something.
- **All-time IOC rollup** — distinct C2 IPs, URLs, and Monero wallets extracted across every payload ever captured, not just the currently-buffered ones.
- **Feed search + "NEW" badges** — free-text search across the live feed, plus a badge the first time a given command, username, or password is ever seen.
